...
instance Show GreyMap
    {- 最后一个参数是 greyData, 这里用 `_` 占位符来代替, 这是因为我们不会把它打印出来 -}
                                                           where
    show (GreyMap width height max _) = "GreyMap " ++ show width ++ "x" ++ show height ++ " " ++ show max

matchHeader :: BL.ByteString -> BL.ByteString -> Maybe BL.ByteString
matchHeader prefix str
    | prefix `BLC8.isPrefixOf` str = Just (BLC8.dropWhile isSpace (BL.drop (BL.length prefix) str))
      -- 去掉 `prefix` 和紧挨着的空格
    | otherwise = Nothing
...
parseP5' s =
    case matchHeader (BLC8.pack "P5") s of
        Nothing -> Nothing
        Just s1 ->
            case getNat s1 of
                Nothing -> Nothing
                Just (width, s2) ->
                    case getNat (BLC8.dropWhile isSpace s2) of
                        Nothing -> Nothing
                        Just (height, s3) ->
                            case getNat (BLC8.dropWhile isSpace s3) of
                                Nothing -> Nothing
                                Just (greyMax, s4)
                                    | greyMax > 255 -> Nothing
                                    | otherwise ->
                                        case getBytes 1 s4 -- 这一个字节还不知道到底是什么
                                              of
                                            Nothing -> Nothing
                                            Just (_, s5) ->
                                                case getBytes (width * height) s5 -- `s5` 就是 `greyData` 部分
                                                      of
                                                    Nothing -> Nothing
                            -- `P5` 格式的 `greyData` 部分可包含多个图像，一个接一个.  `s6 就是剩余还没解析的部分`
                                                    Just (bitmap, s6) -> Just (GreyMap width height greyMax bitmap, s6)
...
instance Functor (ParserWrapper a) where
    fmap f (ParserWrapper parser) = ParserWrapper (parser ==> \result -> identity (f result))
...
parse f initState =
    case f (ParseState initState 0) of
        Left err -> Left err
...
parseByte =
    getState ==> \initState ->
        case BL.uncons (string initState) of
...
parseWhile' f = parserMap (fmap f) peekByte ==> appendByte
  where
    ...
    appendByte (Just True) = parseByte ==> \b -> parserMap (b :) (parseWhile' f)
...
parseNat =
    parseWhile word8ToChar8 isDigit ==> \digits ->
        if null digits
...
--parseP5 :: BL.ByteString -> Maybe (GreyMap, BL.ByteString)
parseP5 =
    parseWhile word8ToChar8 notWhite ==> \header ->
            ...
            skipSpaces ==>& parseNat ==> \height ->
                skipSpaces ==>& parseNat ==> \maxGrey ->
                    parseByte ==>& parseBytes (width * height) ==> \bitmap ->
                        identity (GreyMap width height maxGrey (BL.pack bitmap))
  where
    notWhite :: Char -> Bool
    notWhite = (`notElem` (" \r\n\t" :: String))